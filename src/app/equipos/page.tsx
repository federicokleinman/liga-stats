...
export default function EquiposPage() {
  ...
  if (error || !metrics) {
    return (
      <div className="text-center py-12 text-gray-400">
        <p>{error || 'Error cargando datos.'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              {summary && (
                <div className="flex gap-4 mt-2 text-sm text-gray-400">
                  ...
                  {summary.campeonatos > 0 && (
                    <span className="text-yellow-400">🏆 {summary.campeonatos}</span>
                  )}
      ...
      {filtered.length === 0 && (
        <p className="text-center text-gray-500 py-8">No se encontraron equipos.</p>
      )}
    </div>
  );
}
