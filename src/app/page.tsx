...
export default function HomePage() {
  const { metrics, loading, error, progress } = useMetrics();
      ...
      <div className="space-y-6">
        <div className="bg-[#111827] border border-yellow-600/30 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-400 mb-6">
            {error || 'Error cargando datos. Intentá de nuevo en unos minutos.'}
          </p>
        </div>
      </div>
    );
  ...
  const consistencyCols: Column<(typeof metrics.consistency)[0] & { _rank: number }>[] = [
    ...
    { key: 'temporadas', label: 'Temporadas', align: 'right' },
    {
      ...
      label: 'Prom. Pts',
      align: 'right',
      render: (row) => <span>{row.promedioPuntos.toFixed(1)}</span>,
    },
  ];
  ...
  const streakCols: Column<(typeof metrics.championshipStreaks)[0] & { _rank: number }>[] = [
    ...
    { key: 'divisional', label: 'Div.', align: 'center' },
    {
      key: 'fromTemporada',
      label: 'Período',
      render: (row) => <span>T{row.fromTemporada}–T{row.toTemporada}</span>,
    },
  ];
  ...
  const promoCols: Column<(typeof metrics.promotionStreaks)[0] & { _rank: number }>[] = [
    ...
    { key: 'streak', label: 'Ascensos', align: 'right' },
    {
      key: 'path',
      label: 'Trayecto',
      render: (row) => <span className="text-gray-400">{row.path.join(' → ')}</span>,
    },
    {
      key: 'fromTemporada',
      label: 'Período',
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Liga Universitaria — Fútbol Mayores Masculino</p>
        </div>
        <WhatsAppShare text="Mirá las estadísticas históricas de la Liga Universitaria" />
      </div>
      ...
      <section>
        ...
        <SortableTable
          data={metrics.topChampions.map((r, i) => ({ ...r, _rank: i + 1 }))}
          columns={champCols}
          keyField="teamId"
          defaultSort="campeonatos"
        />
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section>
          ...
          <SortableTable
            data={metrics.championshipStreaks.map((r, i) => ({ ...r, _rank: i + 1 }))}
            columns={streakCols}
            keyField="_rank"
            defaultSort="streak"
          />
        ...
        <section>
          ...
          <SortableTable
            data={metrics.bestSeasons.map((r, i) => ({ ...r, _rank: i + 1 }))}
            columns={bestSeasonCols}
            keyField="_rank"
            defaultSort="puntos"
          />
        </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section>
          ...
          <SortableTable
            data={metrics.bestAttack.map((r, i) => ({ ...r, _rank: i + 1 }))}
            columns={attackCols}
            keyField="_rank"
            defaultSort="value"
          />
        ...
        <section>
          ...
          <SortableTable
            data={metrics.bestDefense.map((r, i) => ({ ...r, _rank: i + 1 }))}
            columns={defenseCols}
            keyField="_rank"
            defaultSort="value"
            defaultDir="asc"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section>
          ...
          <SortableTable
            data={metrics.consistency.map((r, i) => ({ ...r, _rank: i + 1 }))}
            columns={consistencyCols}
            keyField="teamId"
            defaultSort="promedioPuntos"
          />
        </section>
        ...
        <section>
          ...
          <SortableTable
            data={metrics.promotionStreaks.map((r, i) => ({ ...r, _rank: i + 1 }))}
            columns={promoCols}
            keyField="_rank"
            defaultSort="streak"
          />
