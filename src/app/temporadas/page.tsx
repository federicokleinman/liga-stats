...
import { useMetrics, useStandings, useTorneo } from '@/lib/hooks';
...
function TemporadasContent() {
  ...
  const { metrics, loading: metricsLoading, progress } = useMetrics(torneo);
  ...
  if (!metrics) {
    return <div className="text-center py-12 text-gray-400">Error cargando datos.</div>;
  }

  const divs = metrics.divisionales;
  const temps = metrics.allTemporadas;
  const isNumericDivs = divs.every((d) => /^\d+$/.test(d));
  const divLabel = (d: string) => isNumericDivs ? `Serie ${d}` : `Div. ${d}`;
      ...
      {selectedTemp && (
        <div>
          {standingsLoading ? (
            ...
            divs.map((d) => {
              const divRows = rows.filter((r) => r.divisional === d);
